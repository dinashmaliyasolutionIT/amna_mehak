...
for url in route_links:
    ...
    try:
        ...
        match = re.search(r"([A-Za-z\s]+)\s+(?:to|–|-)\s+([A-Za-z\s]+)", route_title)
...
def extract_route(title):
    if not isinstance(title, str):
        return (None, None)
    match = re.search(r"([A-Za-z\s]+)\s+(?:to|–|-)\s+([A-Za-z\s]+)", title)
    if match:
